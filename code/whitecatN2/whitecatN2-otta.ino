...
// ttn otaa whitecatboard N2 amb bme280
// guarda dades entre despertars, no cal re-join
...
// https://github.com/jordibinefa/IoT-02/blob/master/codes/IoT-02-09_ttn-otaa_01/IoT-02-09_ttn-otaa_01.ino
...
//TTS application: whitecat1, device: eui-0000000,
//device address: 0000000
#define TTN_APPEUI { 0x00, 0x00, 0x04, 0xD0, 0x7E, 0xD5, 0xB3, 0x70} // TTN Application EUI with "lsb"
#define TTN_DEVEUI { 0x00, 0xAF, 0x04, 0x00, 0x7E, 0xD5, 0xB3, 0x70 } // TTN Device EUI with "lsb"
#define TTN_APPKEY { 0x00, 0x04, 0x0E, 0x07, 0x4B, 0xAD, 0xFF, 0x00, 0xFB, 0x55, 0x32, 0xE9, 0xA5, 0x04, 0x15, 0x8C } // TTN App Key with "msb"

const unsigned TX_INTERVAL = 120;//temps que dorm l'esp32 en segons
bool GOTO_DEEPSLEEP = false;//variable
...
RTC_DATA_ATTR lmic_t RTC_LMIC;//memoria on es guarda la pila entre dormides

//pins de connexio entre esp32 i la radio en la WC
#define PIN_LMIC_NSS 9
...
// Llibreria pel sensor
#include "Seeed_BME280.h"
#include <Wire.h>
BME280 bme280;
int temperaturaBME280=0;
int humitatBME280=0;
int presioBME280=0;
...
void setup()
{
    ...
    delay(200);//cal esperar a que el bme estigui OK
...
// Ara enviem un paquet i anem a dormir
void loop()
{
    static unsigned long lastPrintTime = 0;
    os_runloop_once();

    //espero a poder anar a dormir!!
...
// Aquesta funcio s'encarrega 
void do_send(osjob_t *j)
{
  readBME280();
  int vbat = analogRead(35);
  Serial.print("VBAT = ");
  Serial.println(vbat); 
  byte longitudPacket=6;
  byte dades[longitudPacket];
  memset(dades, 0, longitudPacket); //esborra estat
  dades[0] = lowByte(temperaturaBME280);
  dades[1] = highByte(temperaturaBME280);
  dades[2] = lowByte(humitatBME280);
  dades[3] = highByte(humitatBME280);
  dades[4] = lowByte(vbat);
  dades[5] = highByte(vbat);

  Serial.print("dades[] = ");
  char bufferKK[longitudPacket];
  for (int i = 0; i < longitudPacket; i++) {
    sprintf(bufferKK, "%02x", dades[i]);
    Serial.print(bufferKK);
  }
  Serial.println(" ");
    ...
    else
    {
        ...
        LMIC_setTxData2(2, dades, sizeof(dades), 0);//port 2
...
void readBME280()
{
  ...
  temperaturaBME280=int(t*100);
  humitatBME280=int(h*100);
  presioBME280=int(p/100);
  Serial.print("t= ");
  Serial.print(t);
  Serial.print(", h= ");
  Serial.print(h);
...
void onEvent(ev_t ev)
{
    ...
    case EV_JOINING:
        ...
        DR_SF7  = EU868_DR_SF7,
        */
        //LMIC_setAdrMode(1); //activat
        //LMIC_setDrTxpow(EU868_DR_SF12,14);
    ...
    case EV_TXCOMPLETE:
        ...
        if (LMIC.dataLen)
        {
            ...
            if(LMIC.frame[LMIC.dataBeg] == 0xAA)
            {
              Serial.println("Rele ON");
              //digitalWrite(2,HIGH);
            }
            if(LMIC.frame[LMIC.dataBeg] == 0xBB)
            {
